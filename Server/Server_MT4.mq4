...
#property copyright "Copyright 2023, Yousuf Mesalm"
#property link "https://www.Yousuf-mesalm.com/"
#property version "1.0"
#property strict
#include "hash.mqh"
#include "json.mqh"
#include "Mql4Aurh.mqh"
...
string delimiter = "|";
string folderName = "YM";
string filePathOrders = folderName + "/_Orders.txt";
string fileSubscriptSymbols = folderName + "/_Subscribed_Symbols.txt";
string filePathMessages = folderName + "/_Messages.txt";
string filePathMarketData = folderName + "/_Market_Data.txt";
string filePathBarData = folderName + "/_Bar_Data.txt";
string filePathHistoricData = folderName + "/_Historic_Data.txt";
string filePathHistoricTrades = folderName + "/_Historic_Trades.txt";
string filePathCommandsPrefix = folderName + "/_Commands_";
string filePathTabs = folderName + "/_Tabs.txt";
...
int OnInit()
{
   ...
   string path = TerminalInfoString(TERMINAL_DATA_PATH) + "\\MQL4\\Experts\\Server\\";
   string Server = "Server.exe"; // replace with the path to your executable
    int r = ShellExecuteW(0, "Open", path + Server, "", path, 1);
    if (r <= 32)
   {
   ...
   // // Let's do it!
   //    if(auth("78093179","1MUSD$!MayB","SERVER IP/HOSTNAME"))
   //      {
...
void CheckCommands()
{
   ...
   for (int i = 0; i < maxCommandFiles; i++)
   {
      ...
      {
         if (magicUpdate == false)
         {
            UpdatSubscribedSymbols(filePathTabs);
            magicnumber = getMagicNumber();
            magicUpdate = true;
         }
         OpenOrder(content, magicnumber);
         // return;
      }
...
//|                                                                  |
//+------------------------------------------------------------------+
void OpenOrder(string orderStr, int magicNumber)
{
   ...
   int magic = StringToInteger(data[9]) > 10000 ? StringToInteger(data[9])
                                                : magicNumber;
   Print(data[9]);
   ...
   if (price == 0 && orderType == OP_BUY)
   {
      ...
      takeProfit = price + takeProfit * MarketInfo(symbol, MODE_POINT) * 10;
   }
   ...
   // add pips to price if price > or < 0 with pending orders
   if (price > 0 && orderType == OP_BUYSTOP)
      price = price * MarketInfo(symbol, MODE_POINT) * 10 + MarketInfo(symbol, MODE_ASK);
   if (price > 0 && orderType == OP_SELLSTOP)
      price = MarketInfo(symbol, MODE_BID) - price * MarketInfo(symbol, MODE_POINT) * 10;
   if (price > 0 && orderType == OP_BUYLIMIT)
      price = MarketInfo(symbol, MODE_ASK) * 10 - price * MarketInfo(symbol, MODE_POINT);
   if (price > 0 && orderType == OP_SELLLIMIT)
      price = price * MarketInfo(symbol, MODE_POINT) + MarketInfo(symbol, MODE_BID) * 10;
...
void CheckMarketData()
{
   ...
   for (int i = 0; i < ArraySize(MarketDataSymbols); i++)
   {
      if (!first)
         text += ", ";
      MqlTick lastTick;
      GetSymbolMagicNumber(MarketDataSymbols[i]);
      bool first1 = true;
      if (SymbolInfoTick(MarketDataSymbols[i], lastTick))
      {
         text += StringFormat("\"%s\":{",
                              MarketDataSymbols[i]);
         for (int m = 0; m < ArraySize(CyclesNumbersArray); m++)
         {
            if (!first1)
               text += ", ";

            text += StringFormat("\"%i\": { \"profit\": %.5f }",
                                 CyclesNumbersArray[m],
                                 getProfit(MarketDataSymbols[i]));
            first = false;
         }
         text += "},";
      }
...
void CheckOpenOrders()
{
   ...
   bool first = true;
   string text = StringFormat("{\"account_info\": {\"name\": \"%s\", \"number\": %d, \"server\": \"%s\",\"currency\": \"%s\", \"leverage\": %d, \"free_margin\": %f, \"balance\": %f, \"equity\": %f, \"profit\": %f}, \"orders\": {",
                              AccountName(), AccountNumber(), AccountServer(), AccountCurrency(), AccountLeverage(), AccountFreeMargin(), AccountBalance(), AccountEquity(), AccountProfit());
...
{
   Print("INFO: " + message);
   message = StringFormat("{\"type\": \"INFO\", \"time\": \"%s %s\", \"message\": \"%s\", \"isUnRead\": \"%s\"}",
                          TimeToString(TimeGMT(), TIME_DATE), TimeToString(TimeGMT(), TIME_SECONDS), message, "true");
   SendMessage(message);
}
...
//+------------------------------------------------------------------+
// function to get all market watch symbols
string getMarketWatchSymbols()
{
...
//|                                                                  |
//+------------------------------------------------------------------+
void SendMarketWatchSymbols()
{

   string text = getMarketWatchSymbols();
   WriteToFile(fileSubscriptSymbols, text);
}
//+------------------------------------------------------------------+
// function to get profit of specified symbol
double getProfit(string symbol)
{
   double profit = 0;
...
// function to read data from tab file and return as string array
void UpdatSubscribedSymbols(string filePath)
{
   string text = "";

   if (!FileIsExist(filePath))
      return;
   int handle = FileOpen(filePath, FILE_READ); // FILE_COMMON |
                                               // Print(filePath, " | handle: ", handle);
   if (handle == -1)
      return;
   if (handle == 0)
   ...
   while (!FileIsEnding(handle))
      text += FileReadString(handle);
   FileClose(handle);
   // search for [] andreplace with ""
   StringReplace(text, "[]", "\"\"");

   JSONParser *parser = new JSONParser();
   JSONValue *TabsValue = parser.parse(text);
   if (TabsValue == NULL)
   {
   ...
   {
      // Print("PARSED:" + TabsValue.toString());

      if (TabsValue.isObject())
      {
         // check root value is an object. (it can be an array)

         JSONObject *jo = TabsValue;
         // Direct access - will throw null pointer if wrong getter used.
         string activeTab = jo.getObject("activeTabSymbol").toString();
         activetavid = jo.getInt("ActiveTab");
         ...
         // string s = "";
         // if (jo.getObject("lotEnd"))
         //    Print("lotEnd = " + s);
         JSONValue *ActiveTab = parser.parse(activeTab);

         // Loop over object keys
         ArrayFree(MarketDataSymbols);
         JSONIterator *it = new JSONIterator(ActiveTab);
         for (; it.hasNext(); it.next())
         {
            int j;
            bool isExist = false;
            for (j = 0; j < ArraySize(MarketDataSymbols); j++)
            {
               if (MarketDataSymbols[j] == it.key())
               {
                  isExist = true;
                  break;
               }
            }
            // check if symbo is true then add it to the array
            MqlTick lastTick;

            if (SymbolInfoTick(it.key(), lastTick))
            {
               if (!isExist)
               {
                  ArrayResize(MarketDataSymbols, ArraySize(MarketDataSymbols) + 1);
                  MarketDataSymbols[ArraySize(MarketDataSymbols) - 1] = it.key();
               }
            }
         }
         delete it;
...
// function generate a new magic number every time
int activetavid = 0;
int getMagicNumber()
{
   int StarticmagicNumber = activetavid * 10000;
   int magicNumber = 0;
   int total = OrdersTotal();
   int i;
   for (i = 0; i < total; i++)
   {
      if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
      {
         if (OrderMagicNumber() > StarticmagicNumber && OrderMagicNumber() < (StarticmagicNumber + 10000) && OrderMagicNumber() > magicNumber)
         {
   ...
   return (MathMax(magicNumber + 1, StarticmagicNumber + 1));
}

// function open new order if order hits takeprofit with same magic number
bool openNewOrder(string symbol, int type, double lots, double price, double stoploss, double takeprofit, int magicNumber, string comment)
{
   int ticket = OrderSend(symbol, type, lots, price, SlippagePoints, stoploss, takeprofit, comment, magicNumber, 0, clrNONE);
   if (ticket > 0)
   {
   ...
   {
      Print("Order failed to open, Error code: ", GetLastError());
      return false;
   }
}
// function check history for last order closedby tp
bool IsOrderClose()
{
      ...
      if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY))
      {
         if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY))
         {
            ...
            if (CloseTime > lastCloseTime)
            {
               ...
               lastMagicNumber = OrderMagicNumber();

               lastTp =
                   OrderType() == OP_BUY ? OrderTakeProfit() - OrderOpenPrice() : OrderOpenPrice() - OrderTakeProfit();
               LastComment = OrderComment();

               // check if the order closed by tp
               if (OrderType() == OP_BUY)
               {
                  if (OrderClosePrice() >= OrderTakeProfit())
                  {
                     // check if order comment didn't contain (END) or (PO)
                     if (StringFind(OrderComment(), "END") < 0 && StringFind(OrderComment(), "PO") < 0)
                     {
                        // GET the number between()from the comment
                        string number = StringSubstr(OrderComment(), StringFind(OrderComment(), "(") + 1,
                                                     StringFind(OrderComment(), ")") - StringFind(OrderComment(), "(") - 1);

                        // increase the number by 1 then add it to the comment
                        number = IntegerToString(StringToInteger(number) + 1);
                        // update commmen with the new number
                        LastComment = StringSubstr(OrderComment(), 0, StringFind(OrderComment(), "(")) + "(" + number + ")" + StringSubstr(OrderComment(), StringFind(OrderComment(), ")") + 1, StringLen(OrderComment()) - StringFind(OrderComment(), ")") - 5);
                        string Lots[];
                        int tp1 = 0;
                        int LotEnd = 0;
                        string Auto = "";
                        getCycleData(OrderSymbol(), lastMagicNumber, Lots, tp1, LotEnd, Auto);
                        if (Auto == "true")
                        {
                           Print("Auto=true");
                           // llop on array and get the next lot
                           lastLotSize = StringToDouble(Lots[(int)StringToInteger(number)]);
                           lastLotSize = lastLotSize < LotEnd ? lastLotSize : LotEnd;
                           //  open new order with same parameters
                           bool OrderOpend = false;
                           while (!OrderOpend)
                           {
                              double p = SymbolInfoDouble(OrderSymbol(), SYMBOL_ASK);
                              if (openNewOrder(OrderSymbol(), OP_BUY, lastLotSize, p, 0, p + tp1 * MarketInfo(OrderSymbol(), MODE_POINT) * 10,
                                               lastMagicNumber, LastComment))
                                 OrderOpend = true;
                           }
                        }
                     }
                     // check if order comment contain(END)
                     else if (StringFind(OrderComment(), "END") > 0)
                     {
                        // ckose allorder with the same magic number
                        closeAllOrders(lastMagicNumber, OrderSymbol());
                     }
                  }
               }
               else
                  ...
                  if (OrderClosePrice() <= OrderTakeProfit())
                  {
                     if (StringFind(OrderComment(), "END") < 0 && StringFind(OrderComment(), "PO") < 0)
                     {
                        ...
                        if (Auto == "true")
                        {
                           ...
                           //  open new order with same parameters
                           bool OrderOpend = false;
                           double p = SymbolInfoDouble(OrderSymbol(), SYMBOL_BID);
                           openNewOrder(OrderSymbol(), OP_SELL, lastLotSize, p, 0, p - tp1 * MarketInfo(OrderSymbol(), MODE_POINT) * 10,
                                        lastMagicNumber, LastComment);
                        }
                     } // check if order comment contain(END)
                     else if (StringFind(OrderComment(), "END") > 0)
...
//|                                                                  |
//+------------------------------------------------------------------+
void closeAllOrders(int magicNumber, string symbol)
{
   {
...
// function return magic number of symbol
int CyclesNumbersArray[];
void GetSymbolMagicNumber(string symbol)
{
   ArrayFree(CyclesNumbersArray);
   int total = OrdersTotal();
   int i = 0;
   for (i = 0; i < total; i++)
   {
      if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
      {
         if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
         {
            if (OrderSymbol() == symbol)
            {
               // check if the magic number is not in the array
               bool found = false;
               for (int j = 0; j < ArraySize(CyclesNumbersArray); j++)
               {
                  if (CyclesNumbersArray[j] == OrderMagicNumber())
                  {
                     found = true;
                  }
               }
               ...
               if (!found)
               {
                  ArrayResize(CyclesNumbersArray, ArraySize(CyclesNumbersArray) + 1);
                  CyclesNumbersArray[ArraySize(CyclesNumbersArray) - 1] = OrderMagicNumber();
               }
            }
         }
      }
...
//|                                                                  |
//+------------------------------------------------------------------+
string CyclesMagicsNumber[];
// function get magics numbers
string GetCycleMagics()
{
   ArrayFree(CyclesMagicsNumber);
   string text = "";

   int handle = FileOpen(filePathTabs, FILE_READ); // FILE_COMMON |
   ...
   else
   {
      ...
      if (TabsValue.isObject())
      {
         ...
         if (jo.isObject())
         {
               TotalProfitToClose=StringToDouble(jo.getString("TP"));

            JSONIterator *Cycles = new JSONIterator(jo);
            for (; Cycles.hasNext(); Cycles.next())
            {
               
               if (Cycles.key() == "Cycles")
               {
                  
                  if (Cycles.val().toString() != "{}")
                {
                     JSONIterator *symbols = new JSONIterator(Cycles.val());
                     if (symbols.val().isObject())
                        for (; symbols.hasNext(); symbols.next())
                        {
                           // Print(symbols.val());
                           JSONIterator *magix = new JSONIterator(symbols.val());
                           for (; magix.hasNext(); magix.next())
                           {
                              ArrayResize(CyclesMagicsNumber, ArraySize(CyclesMagicsNumber) + 1);
                              CyclesMagicsNumber[ArraySize(CyclesMagicsNumber) - 1] = magix.key();
                           }
                           delete magix;
                        }
                     delete symbols;
         ...
         // string lotArraystr = jo.getObject("Cycles").getObject(symbol).getObject(IntegerToString(magic)).getArray("lotArray").toString();
         // // split the string and add it to the array
         // ushort seperator = StringGetCharacter(",", 0);
         // StringSplit(lotArraystr, seperator, lotArray);
         // tp = jo.getObject("Cycles").getObject(symbol).getObject(IntegerToString(magic)).getInt("TP1");
         // lotEnd = jo.getObject("Cycles").getObject(symbol).getObject(IntegerToString(magic)).getInt("lotEnd");
...
double TotalProfitToClose = 0;
void getCycleData(string symbol, int magic, string &lotArray[], int &tp, int &lotEnd, string &Auto)
{
   ...
   else
   {
      ...
      if (TabsValue.isObject())
      {
         ...
         tp = jo.getObject("Cycles").getObject(symbol).getObject(IntegerToString(magic)).getInt("TP1");
         lotEnd = jo.getObject("Cycles").getObject(symbol).getObject(IntegerToString(magic)).getInt("lotEnd");
         Auto = jo.getObject("Cycles").getObject(symbol).getObject(IntegerToString(magic)).getString("Auto");
         TotalProfitToClose = StringToDouble(jo.getString("TP"));
         Print(TotalProfitToClose);
         // //    Print("city:" + jo.getObject("address").getString("city"));
         // //    Print("phone:" + jo.getArray("phoneNumber").getObject(0).getString("number"));
...
// loop through all orders and upte the cycle orders if tabs upfate
void UpdateCycleOrders()
{
   for (int i = 0; i < ArraySize(CyclesMagicsNumber); i++)
   {

      int magic = StringToInteger(CyclesMagicsNumber[i]);
      string symbol = "";
      int total = OrdersTotal();
      int j = 0;
      for (j = 0; j < total; j++)
         ...
         if (OrderSelect(j, SELECT_BY_POS, MODE_TRADES))
         {
            if (OrderSelect(j, SELECT_BY_POS, MODE_TRADES))
            {
               if (OrderMagicNumber() == magic)
               {
                  ...
                  double TP1 = OrderType() == OP_BUY ? (OrderTakeProfit() - OpenPrice) / (MarketInfo(OrderSymbol(), MODE_POINT) * 10) : OrderType() == OP_BUY ? (OpenPrice - OrderTakeProfit()) / (MarketInfo(OrderSymbol(), MODE_POINT) * 10)
                                                                                                                                                              : 0;
                  string Commant = OrderComment();
                  symbol = OrderSymbol();
                  if (StringFind(Commant, "END") < 0 && StringFind(Commant, "PO") < 0)
                  {
                     string Lots[];
                     int tp1 = 0;
                     int LotEnd = 0;
                     string Auto = "";
                     getCycleData(symbol, magic, Lots, tp1, LotEnd, Auto);
                     Print(tp1);
                  }
               }
            }
      ...
      //    int tp1 = 0;
      //    int LotEnd = 0;
      //    getCycleData(symbol, magic, Lots, tp1, LotEnd);
      //    // llop on array and get the next lot
      //    lastLotSize = StringToDouble(Lots[0]);
      //    lastLotSize = lastLotSize < LotEnd ? lastLotSize : LotEnd;
      //    //  open new order with same parameters
      //    bool OrderOpend = false;
      //    double p = SymbolInfoDouble(symbol, SYMBOL_ASK);
      //    openNewOrder(symbol, OP_BUY, lastLotSize, p, 0, p + tp1 * MarketInfo(symbol, MODE_POINT) * 10,
      //                 magic, LastComment);
      //  }
   }
}
